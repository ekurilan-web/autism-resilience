...
<head>
  ...
  <title>מחוסר לחוסן | חוויה אינטראקטיבית</title>
  
  <meta name="apple-mobile-web-app-capable" content="yes">
  <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
  
  <meta property="og:title" content="🎭 מה מסתתר מאחורי המסכה?" />
  <meta property="og:description" content="חוויה אינטראקטיבית: מסע קצר אל עבר חוסן אוטיסטי אותנטי" />
  <style>
    :root {
    ...
    * { 
    ...
    .node-circle { 
      r: 28; /* הגדלת הרדיוס לעיגולים */
      fill: #1e293b; 
      stroke-width: 2; 
    }
    .node-text { 
      fill: white; 
      font-size: 9px; /* הקטנת הגופן */
      font-weight: 700; 
      text-anchor: middle; 
...
<body>
  ...
  <div class="header-area">
    <h1 class="title" id="mainTitle">מסיכה או חוסן?</h1>
    <p class="subtitle" id="subTitle">הקישו על המסכה (פעמיים) כדי לגלות</p>
    <div class="instruction-pill" id="instruction">הקישו על העיגולים לגלות את שלושת מרחבי החוסן האוטיסטי 👇</div>
  </div>
  <div class="container">
    <div class="scene">
  ...
  <div class="footer" id="footer">
    ...
    <p style="margin-top: 10px; font-size: 0.8rem; color: #64748b; padding: 0 20px;">
      מחוסר לחוסן: המשגה נוירואפירמטיבית בכנס השיקומי
    </p>
    <button class="reset-btn" onclick="resetExperience()">🔄 להתחיל מחדש</button>
  </div>

  <div class="popup-overlay" id="popup">
    <div class="popup-card">
      <div class="popup-tag" id="pTag">קטגוריה</div>
      <h3 class="popup-title" id="pTitle">כותרת</h3>
      <p class="popup-subtitle" id="pSubtitle">כותרת משנה</p>
      <p class="popup-desc" id="pDesc">הסבר</p>
      <button class="close-btn" onclick="closePopup()">סגירה</button>
    </div>
  </div>
  ...
  <script>
    ...
    function growRhizome() {
      ...
      concepts.forEach((c, i) => {
        ...
        circle.setAttribute('r', '28'); // רדיוס מוגדל
        circle.setAttribute('fill', '#1e293b');
        circle.setAttribute('stroke', c.color); 
        circle.setAttribute('stroke-width', '2');
        
        // הטקסט - ללא פיצול לשורות
        const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
        text.setAttribute('x', endX); 
        text.setAttribute('y', endY);
        text.setAttribute('class', 'node-text');
        text.setAttribute('font-size', '9'); // גופן קטן יותר
        
        // טקסט רגיל (ללא \n)
        text.textContent = c.label;
        
        // מיקום טקסט מדויק יותר
        if (c.label.length > 6) { // אם הטקסט ארוך יותר מ-6 תווים
          text.setAttribute('font-size', '8'); // גופן עוד יותר קטן
        }
